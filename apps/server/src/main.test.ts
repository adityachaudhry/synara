  ...
  it.effect("omits both server authority secrets from startup log data", () =>
    Effect.gen(function* () {
      yield* runCli([], {
        ...
        SUPERTOKENS_API_KEY: "supertokens-secret",
        SUPERTOKENS_API_DOMAIN: "https://glasswing.example.test",
        SUPERTOKENS_WEBSITE_DOMAIN: "https://glasswing.example.test",
      });
      ...
      assert.notInclude(JSON.stringify(logData), "supertokens-secret");
    }),
  );
