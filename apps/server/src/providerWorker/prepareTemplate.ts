...
import {
  WORKER_TOOLCHAIN_CHECK_COMMAND,
  WORKER_TOOLCHAIN_INSTALL_COMMAND,
} from "./workerToolchain.ts";
...
export async function prepareProviderWorkerTemplate(artifactPath: string) {
  ...
  if (!config.enabled || !config.railway.enabled)
    throw new Error("Railway runtime is not configured.");
  ...
  try {
    const tools = await sandbox.exec(WORKER_TOOLCHAIN_CHECK_COMMAND, { timeoutSec: 15 });
    if (tools.exitCode !== 0) {
      const installed = await sandbox.exec(WORKER_TOOLCHAIN_INSTALL_COMMAND, { timeoutSec: 180 });
...
if (import.meta.main) {
  if (process.env.SYNARA_PREPARE_WORKER_TEMPLATE !== "1") {
    throw new Error(
      "Set SYNARA_PREPARE_WORKER_TEMPLATE=1 to authorize Railway template provisioning.",
    );
  }
