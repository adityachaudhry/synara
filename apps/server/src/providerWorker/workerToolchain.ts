...
export const WORKER_TOOLCHAIN_INSTALL_COMMAND =
  'if [ "$(id -u)" = 0 ]; then apt-get update -qq && apt-get install -y -qq --no-install-recommends git-lfs poppler-utils python3-openpyxl procps; else sudo apt-get update -qq && sudo apt-get install -y -qq --no-install-recommends git-lfs poppler-utils python3-openpyxl procps; fi && (python3 -c \'import openpyxl\' || python3 -m pip install --disable-pip-version-check --no-cache-dir openpyxl==3.1.5)';
