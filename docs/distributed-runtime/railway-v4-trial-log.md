...
### Durable thread recovered, stale worker process retained

**Result:** Deployment `f8fdd69d-9cd4-4648-b9ea-22e350cea4ef` recovered the exact volume without wiping it. The existing authenticated browser cookie and thread `980fedcd-464e-42fd-b007-6659b7ccf685` remained valid, including the earlier `volume backed pi ok` transcript. A follow-up on that same thread returned `durable restart pi ok`, proving the controller journals, projections, settings, auth records, and distributed runtime binding survived replacement and selected the persisted restart seam.
